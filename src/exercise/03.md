...
Use useContext sparingly. This allows you to share state between components as long 
as the components are children of the relevant contextProvider. This is good because it 
allows state to be shared only between the children (without prop drilling, which is the main benefit) that need it, making the app easier to maintain.

Prop drilling is bad because you have to pass the state down mulitple children that dont need it
to get to the child that actually needs it.

However, the issue with useContext is that the state is IMPLIED. Which means that the user of our components
cant see all the dependencies of that state without opening the file. For example, <Component foo={'bar'} />
the user may think foo is the only dependency and if they use the component outside the scope of <ContextProvider>, they wont have access to the state from `useContext`, leading to undesired behaviour.


The alternative to implied state is Component Composition, which makes all the deps. of the component explicit.


When `useContext` is used, make sure to use it via a `useContextHook`  that checks to make sure that the consumer of the state is trying to access the state within the ProviderContext
